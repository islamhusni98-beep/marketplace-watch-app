...
// One country-wide Used page per target. Location is filtered from each card.
const TARGETS=[
  {name:'Nissan Sunny',minYear:2016,url:'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/nissan/model-sunny/',re:/\bnissan\s+sunny\b|\bsunny\b|نيسان\s+صني|صني/i},
  {name:'Chevrolet Aveo',minYear:2016,url:'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chevrolet/model-aveo/',re:/\bchevrolet\s+aveo\b|\baveo\b|شيفروليه\s+افيو|شيفروليه\s+أفيو|افيو|أفيو/i},
  {name:'Chevrolet New Optra',minYear:2016,url:'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chevrolet/model-optra/',re:/\bchevrolet\s+(?:new\s+)?optra\b|\b(?:new\s+)?optra\b|شيفروليه\s+(?:نيو\s+)?اوبترا|شيفروليه\s+أوبترا|اوبترا|أوبترا/i},
  {name:'Hyundai Accent RB',minYear:2016,url:'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/hyundai/q-accent-rb/',re:/\bhyundai\s+accent\s*rb\b|\baccent\s*rb\b|\brb\b|هيونداي\s+اكسنت\s*rb|اكسنت\s*rb|أكسنت\s*rb/i},
  {name:'BYD F3',minYear:2021,url:'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/byd/model-f3/',re:/\bbyd\s*f3\b|\bf3\b|بى\s*واى\s*دى\s*اف\s*3|بي\s*واي\s*دي\s*f3/i},
  ...
  {name:'Chery Arrizo 5',minYear:1990,url:'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chery/model-arrizo-5/',re:/\bchery\s+arrizo\s*5\b|\barrizo\s*5\b|شيري\s+اريزو\s*5|اريزو\s*5/i},
];
...
async function send(i){
  const text=['🚗 إعلان سيارة مستعملة جديد',`🎯 السيارة: ${i.target}`,'🌐 المصدر: Dubizzle','',`📌 ${i.title}`,`📅 السنة: ${i.year}`,`🛣️ الكيلومترات: ${i.km}`,`⚙️ الفتيس: ${i.transmission||'غير ظاهر'}`,`💰 السعر: ${i.price}`,`🕐 نازل من: ${i.ago}`,`📍 المكان: ${i.loc}`,'✅ التحقق: صفحة Used للموديل + القاهرة الكبرى + آخر 3 أيام',`🔗 رابط الإعلان: ${i.url}`].join('\n');
...
let pagesWithCards=0,collected=0,sent=0,duplicates=0,old=0,wrongYear=0,manualRejected=0,parseRejected=0,outside=0,modelRejected=0;
const samples=[];
...
for(const target of TARGETS){
  ...
  try{
    ...
    for(const c of found){
      ...
      if(samples.length<12)samples.push({id:c.id,target:target.name,title,year,transmission,ago,loc,price});
    }
  }catch(e){console.warn(`Dubizzle model ${target.name}: ${e.message}`)}finally{await page.close()}
}
await fs.writeFile(seenPath,JSON.stringify([...seen].slice(-5000),null,2));
console.log(`Dubizzle modelWatch searches=${TARGETS.length}, pagesWithCards=${pagesWithCards}, collected=${collected}, duplicates=${duplicates}, modelRejected=${modelRejected}, wrongYear=${wrongYear}, manualRejected=${manualRejected}, oldOrAgeMissing=${old}, outside=${outside}, parseRejected=${parseRejected}, sent=${sent}`);
if(samples.length)console.log(`Dubizzle modelWatch samples=${JSON.stringify(samples)}`);
await browser.close();