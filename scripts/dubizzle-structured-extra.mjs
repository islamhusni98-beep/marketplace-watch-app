...
const TARGETS=[
  {name:'Chevrolet Lanos',minYear:2016,manualOnly:false,urls:[
    'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chevrolet/model-lanos/giza/',
    'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chevrolet/model-lanos/cairo/'
  ],re:/\bchevrolet\s+lanos\b|\blanos\b|شيفروليه\s+لانوس|لانوس/i},
  {name:'Chery Arrizo 5',minYear:2019,manualOnly:true,urls:[
    'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chery/model-arrizo-5/giza/',
    'https://www.dubizzle.com.eg/en/vehicles/cars-for-sale/used/chery/model-arrizo-5/cairo/'
  ],re:/\bchery\s+arrizo\s*5\b|\barrizo\s*5\b|شيري\s+اريزو\s*5|شيرى\s+أ?ريزو\s*5|اريزو\s*5/i}
];
...
function location(lines,ago){const i=lines.indexOf(ago); if(i<1)return ''; for(let j=i-1;j>=0&&j>=i-6;j--){const x=(lines[j]||'').trim(); if(!x||/^[•·|]$/.test(x))continue; if(/^(benzine|diesel|electric|hybrid|automatic|manual|fuel type|transmission|kilometers|mileage|year|condition|used)$/i.test(x))continue; return x;} return ''}
const isGreaterCairo=t=>/giza|cairo|جيزة|الجيزة|القاهرة|haram|هرم|october|اكتوبر|أكتوبر|zayed|زايد|nasr city|مدينة نصر|new cairo|القاهرة الجديدة|tagamoa|التجمع|heliopolis|مصر الجديدة|maadi|المعادي|madinaty|مدينتي|shorouk|الشروق|helwan|حلوان|ramses|رمسيس/i.test(norm(t));
...
async function send(i){
  const text=['🚗 إعلان سيارة مستعملة جديد',`🎯 السيارة: ${i.target}`, '🌐 المصدر: Dubizzle','',`📌 ${i.title}`,`📅 السنة: ${i.year}`,`🛣️ الكيلومترات: ${i.km}`,`⚙️ الفتيس: ${i.transmission||'غير ظاهر'}`,`💰 السعر: ${i.price}`,`🕐 نازل من: ${i.ago}`,`📍 المكان: ${i.loc}`,'✅ التحقق: صفحة الموديل Used + القاهرة الكبرى + آخر 3 أيام',`🔗 رابط الإعلان: ${i.url}`].join('\n');
...
let pages=0,pagesWithCards=0,collected=0,sent=0,duplicates=0,old=0,wrongYear=0,manualRejected=0,parseRejected=0,outside=0;
const samples=[];
...
for(const target of TARGETS){
  for(const url of target.urls){
    ...
    try{
      ...
      const found=await cards.evaluateAll(nodes=>nodes.map(card=>{
      ...
      console.log(`Dubizzle structured ${target.name}: cards=${found.length}, finalUrl=${page.url()}`);
      collected+=found.length;
      for(const c of found){
        const key=`Dubizzle:${c.id}`; if(seen.has(key)){duplicates++;continue}
        const lines=c.text.split('\n').map(x=>x.trim()).filter(Boolean);
        const title=c.title||''; if(!target.re.test(`${title} ${c.text}`)){parseRejected++;continue}
        ...
        if(samples.length<6)samples.push({id:c.id,target:target.name,title,year,transmission,ago,loc,price});
      }
...
console.log(`Dubizzle structured pages=${pages}, pagesWithCards=${pagesWithCards}, collected=${collected}, duplicates=${duplicates}, wrongYear=${wrongYear}, manualRejected=${manualRejected}, oldOrAgeMissing=${old}, outside=${outside}, parseRejected=${parseRejected}, sent=${sent}`);
